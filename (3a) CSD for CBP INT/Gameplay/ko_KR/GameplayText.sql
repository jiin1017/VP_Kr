...
UPDATE Language_ko_KR
SET Text = '당신은 {1_CityStateName:textkey}의 화를 불러 일으킬 어떤 행동을 했습니다. 아마 최근에 그들의 국경을 무단 침입했거나 전쟁 상태일지도 모릅니다. [NEWLINE][NEWLINE]당신의 [ICON_INFLUENCE]영향력을 턴당 {2_Num} 회복합니다. [ICON_INFLUENCE]영향력을 즉시 회복하기 위해서는 외교 유닛을 보내야만 합니다.'
...
UPDATE Language_ko_KR
SET Text = '유닛이 전쟁 중이 아닌 도시 국가 영토 안에 있다면 이 명령은 유닛을 소모시킬 것입니다.[NEWLINE][NEWLINE]만약 위대한 외교관이면, 도시 국가에 대한 대량의 [ICON_INFLUENCE]영향력을 획득하고 다른 문명의 [ICON_INFLUENCE]영향력을 감소시킵니다.[NEWLINE][NEWLINE]만약 위대한 상인이면, 대량의 [ICON_GOLD]골드를 획득하고 모든 도시가 즉시 "국왕 경축일"을 맞이합니다.'
...
UPDATE Language_ko_KR
SET Text = '국제 연합 프로젝트가 완료되면 세계 대회가 국제 연합으로 바뀌며, 외교 승리 투표를 시작할 수 있고 세계 이념이 활성화됩니다. 글로벌 헤게모니 결의안에 대한 대표단 지지를 충분히 확보해야 [ICON_VICTORY_DIPLOMACY]외교 승리가 가능합니다. 일반 상정 결의안을 다루는 회기와 교대로 이루어지는 세계 지도자를 선택하는 회기 동안에는 세계 지도자 결의안이 대회에 자동으로 상정되며 그 외의 결의안은 상정되지 않습니다. 다른 결의안과는 달리 세계 지도자 결의안은 어느 한 문명이 상정할 수 있는 결의안이 아니라 세계 대회의 2회기 마다 세계 지도자 선택에 대한 논의가 이루어집니다. 일반 상정 결의안을 다루는 회기와 교대로 이루어지는 세계 지도자를 선택하는 회기 동안에는 세계 지도자 결의안이 대회에 자동으로 상정되며 그 외의 결의안은 상정되지 않습니다. [NEWLINE][NEWLINE]외교 승리를 달성하는 데 필요한 대표단 수는 게임 내 문명과 도시 국가 수에 따라 달라지며 세계 대회와 승리 진척도 화면에서 확인할 수 있습니다. 만약 어느 문명도 세계 지도자 결의안의 조건을 충족시킬 정도의 지지를 확보하지 못하면, 가장 많은 지지를 확보한 상위 두 개의 문명이 영구적으로 추가 대표단을 확보하게 됩니다.'
WHERE Tag = 'TXT_KEY_CONGRESS_VICTORY_HEADING2_BODY' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_TEXT' AND Value= 1 );

UPDATE Language_ko_KR
SET Text = '국제 연합 프로젝트가 완료되면 세계 대회가 국제 연합으로 바뀌며 [ICON_VICTORY_DIPLOMACY]외교 승리 투표를 시작할 수 있고 세계 이념이 활성화됩니다. 이 전제 조건들은 세계 지도자 선출 결의안을 상정 하고 그 결의안의 승자는 외교적인 승리를 얻을 것입니다.'
WHERE Tag = 'TXT_KEY_CONGRESS_UN_HEADING2_BODY' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_TEXT' AND Value= 1 );


UPDATE Language_ko_KR
SET Text = '새로운 신도 수가 가장 많은 문명에게 보상하겠다고 합니다.'
WHERE Tag = 'TXT_KEY_CITY_STATE_QUEST_CONTEST_FAITH_FORMAL' AND EXISTS (SELECT * FROM CSD WHERE Type='CSD_TEXT' AND Value= 1 );

UPDATE Language_ko_KR
SET Text = '{TXT_KEY_CITY_STATE_QUEST_CONTEST_FAITH_FORMAL} 현재 1위 문명의 새로운 신도 수는 {1_LeaderScore}이며 당신의 새로운 신도 수는 [COLOR_POSITIVE_TEXT]{2_PlayerScore}[ENDCOLOR]입니다.'
...
UPDATE Language_ko_KR
SET Text = '{TXT_KEY_CITY_STATE_QUEST_CONTEST_FAITH_FORMAL} 현재 당신의 문명이 1위이며 새로운 신도 수는 [COLOR_POSITIVE_TEXT]{1_PlayerScore}입니다.'
